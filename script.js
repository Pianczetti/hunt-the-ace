const cardObjectDefinitions = [
    ...
    {id: 3, imagePath: 'images/card-AceSpades.png'},
...
function outputChoiceFeedBack(hit){
    ...
    if(hit){
        setTimeout(()=>{updateStatusElement(currentGameStatusElem, "block", winColor, "Hit!! - Well Done! :)")}, 500)
    }
    else{
        setTimeout(()=>{updateStatusElement(currentGameStatusElem, "block", loseColor, "Missed!! :(")}, 500)
    }
}
...
function checkForIncompleteGame(){
    ...
    if(serializedGameObj){
        ...
        if(gameObj.round >= maxRounds){
            ...
            if(confirm('Would you like to continurer with your last game?')){
                score = gameObj.score;
...
function createCard(cardItem) {
    ...
    //add class to div-front and back
    ...
    attatchClickEventHandlerTo(cardElem);
}

function attatchClickEventHandlerTo(card){
...
function addCardToGridCell(cardItem){

    const cardPositionClassName = mapCardIdToGridCell(cardItem);

    const cardPosElem = document.querySelector(cardPositionClassName);

    addChildElement(cardPosElem, cardItem)
